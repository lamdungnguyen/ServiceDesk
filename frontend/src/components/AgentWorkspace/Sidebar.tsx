...
import { HeadphonesIcon, LayoutList, Loader, CheckCircle2, AlertTriangle, Activity } from 'lucide-react';


type TabId = 'assigned' | 'in_progress' | 'resolved' | 'overdue' | 'activity';
...
const NAV_ITEMS: { id: TabId; label: string; icon: React.ReactNode; danger?: boolean }[] = [
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2.5">
                <span className={isActive ? (item.danger ? 'text-red-600' : 'text-blue-600') : 'text-slate-400'}>
                  {item.icon}
                </span>
                {item.label}
              </div>
